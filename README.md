...
```
## Updates
Example traces availale to download via google drive ([link](https://drive.google.com/file/d/1e_8olEFCne_E5Sia4ThYPAbqSIwUKrwT/view?usp=sharing)).

## 1. Installation
This repository uses python3.9–3.11. To install dependendies run:
...
```
### 2.3 Output
The generator is going to output several synthetic traces in the order of `input.json`. For example, if the first trace in the `input.json` is from MEX, then `trace_0.txt` generated by SpaceGEN is MEX's trace. In each trace, every request is in the format of `time,id,size(KB)`. We cannot release the detailed traffic rate from the original traces due to sensitivity. 
